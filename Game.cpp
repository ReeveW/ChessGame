...
Game::Game() : window(sf::VideoMode(800, 800), "elijah destroyer v1") {}

void Game::run() {
  summonStartingSprites();  // summons all the sprites in their starting
                            // positions
  board.generateAllMoves();

  while (window.isOpen()) {
    sf::Event event{};  // SFML thing that stores an "event" which is like a
                        // user action
    while (window.pollEvent(
        event)) {  // continuously checks for new events each frame. if there is
                   // one, pollEvent stores it as event then we handle it
      if (event.type == sf::Event::Closed) {
        // if user closes window then we close the window cause for some reason
        // SFML doesn't do that automatically ig
        window.close();
      } else if (isPromoting) {
        choosePromotionPiece(event);
      } else {
        handleDragAndDrop(event);  // only other event we handle is dragging and
                                   // dropping pieces so  we send the event
                                   // there if they weren't closing the window
      }
    }

    updateDragPosition();  // makes sprite move with mouse when drag and
                           // dropping

    window.clear();
    for (auto& s : sprite) {
      window.draw(s);  // draws all the sprites summoned by
                       // summonStartingSprites(); (as well as the chessboard)
    }
...
void Game::handleDragAndDrop(
    sf::Event& event) {  // this function 100% needs updating. It's doing too
                         // many things. should be split up into multiple
                         // different functions
  ...
  if (event.type == sf::Event::MouseButtonReleased &&
    ...
    if (draggedPiece) {
      ...
      if (!foundMove) {
        draggedPiece->setPosition((prevIndex % 8) * 100.f,
                                  (prevIndex / 8) * 100.f);
      }
    }
    ...
    if (i == 1 || i == 7) {  // loop to make 8 pawn sprites for each side
      for (int k = 0; k < 8; ++k) {
        ...
        j++;
               ...
               i == 10) {  // loop for rooks, knights and bishops to make two of
                           // each
      for (int k = 0; k < 2; ++k) {
        sprite[j].setTexture(texture[i]);
        j++;
